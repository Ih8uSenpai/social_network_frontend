...
export const CropTest: React.FC<CropTestProps> = ({profile, token, fetchProfile}) => {
    ...
    const handleClose = () => {
        setCropModalOpen(false); // Закрытие модального окна
    };
    ...
    const onDownloadCropClick = async () => {
        ...
        previewCanvasRef.current.toBlob(async (blob) => {
            ...
            try {
                const response = await fetch(`http://localhost:8080/api/profiles/upload-icon/${profile.user.userId}`, {
                    method: 'POST',
                    headers: {
